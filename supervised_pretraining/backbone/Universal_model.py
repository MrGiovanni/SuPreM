...
class Universal_model(nn.Module):
    """
    A flexible segmentation model supporting multiple backbones and organ encoding schemes.

    Args:
        img_size (tuple): The expected input image size.
        in_channels (int): The number of input channels (e.g., 1 for grayscale).
        out_channels (int): The number of output classes (i.e., organs).
        backbone (str, default='swinunetr'): The desired backbone architecture. Options include:
             * 'swinunetr': A Swin Transformer-based UNETR architecture.
             * 'unet': A conventional 3D U-Net architecture.
             * 'dints': DiNTS architecture (refer to external library documentation for details)
        encoding (str, default='rand_embedding'): The type of organ encoding. Supports:
             * 'rand_embedding':  Randomly initialized organ embeddings.
             * 'word_embedding': Pre-trained text-based organ embeddings.
    """
    def __init__(
    ...
    def forward(self, x_in):
        ...
        Args:
            x_in (Tensor): The input data tensor.
        ...
        b = x_feat.shape[0]
        logits_array = []
        for i in range(b): # iterate over samples in batch
            
            # concatenate features with organ encodings
            x_cond = torch.cat([x_feat[i].unsqueeze(0).repeat(self.class_num,1,1,1,1), task_encoding], 1)
            
            # controller generates parameters for dynamic heads
            params = self.controller(x_cond)
            params.squeeze_(-1).squeeze_(-1).squeeze_(-1)
            
            # pre-classifier convolution applied to individual predictions
            ...
            # dynamically parameterized convolutional operations 
