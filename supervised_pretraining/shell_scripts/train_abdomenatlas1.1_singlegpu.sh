#!/bin/bash
#SBATCH --job-name=abdomenatlas1.1-singlegpu
...
# Single GPUs
python -W ignore -m torch.distributed.launch --nproc_per_node=$nproc_per_node --master_port=$RANDOM_PORT train.py --data_root_path $datapath --dataset_list $datasetversion --num_workers $num_workers --log_name $datasetversion.$1.singlegpu --word_embedding $wordembeddingpath --backbone $1 --lr 1e-3 --warmup_epoch 20 --batch_size $batch_size --max_epoch 2000 --cache_dataset --num_class 25 --cache_num $cache_num --dataset_version $datasetversion
