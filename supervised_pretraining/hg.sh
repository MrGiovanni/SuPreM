...
# # Multiple GPUs
# # Pre-train models with the swinunetr and the unet backbones
...
# # Pre-train models with the segresnet backbone
# python -W ignore -m torch.distributed.launch --nproc_per_node=$nproc_per_node --master_port=$RANDOM_PORT train.py --dist --data_root_path $datapath --dataset_list $datasetversion --num_workers $num_workers --log_name $datasetversion.$1 --backbone $1 --lr 1e-4 --warmup_epoch 20 --batch_size $batch_size --max_epoch 1600 --cache_dataset --num_class 9 --cache_num $cache_num --dataset_version $datasetversion

# # for backbone in segresnet; do sbatch --error=logs/$backbone.1.0.out --output=logs/$backbone.1.0.out hg.sh $backbone; done
...
# for backbone in segresnet; do sbatch --error=logs/$backbone.1.1.out --output=logs/$backbone.1.1.out hg.sh $backbone; done
