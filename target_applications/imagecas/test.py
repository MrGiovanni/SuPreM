...
def validation(model, ValLoader, args):
    ...
    if dice_results:
        with open(os.path.join(output_directory, f'{args.pretraining_method_name}_{args.model_backbone}_{args.map_type}_dice_validation_results.csv'), 'w', newline='') as file:
            fieldnames = ["name", "background"] + list(selected_class_map.values())
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(dice_results)
    if nsd_results:
        with open(os.path.join(output_directory, f'{args.pretraining_method_name}_{args.model_backbone}_{args.map_type}_nsd_validation_results.csv'), 'w', newline='') as file:
            fieldnames = ["name", "background"] + list(selected_class_map.values())
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
