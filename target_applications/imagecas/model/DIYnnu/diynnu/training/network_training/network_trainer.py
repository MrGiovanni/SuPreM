...
class NetworkTrainer(object):
    ...
    def run_training_dis(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            train_losses_epoch_su = []
            train_losses_epoch_un = []
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        self.tr_gen.it = b 
                        l, l_su, l_un, moment_t = self.run_iteration(self.tr_gen, True)

                        tbar.set_postfix(loss=l, loss_su=l_su, loss_un=l_un, moment_t=moment_t)
                        train_losses_epoch_su.append(l_su)
                        train_losses_epoch_un.append(l_un)
            ...
            self.all_tr_losses.append(np.mean(train_losses_epoch_su))
            self.print_to_log_file("train su loss : %.4f" % self.all_tr_losses[-1])
            self.print_to_log_file("train un loss : %.4f" % np.mean(train_losses_epoch_un))
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l, l_su, l_un, moment_t = self.run_iteration(self.val_gen, False, True)
                    val_losses.append(l_su)
                ...
                if self.also_val_in_tr_mode:
                    ...
                    val_losses = []
                    for b in range(self.num_val_batches_per_epoch):
                        l, l_su, l_un, moment_t = self.run_iteration(self.val_gen, False)
                        val_losses.append(l_su)
