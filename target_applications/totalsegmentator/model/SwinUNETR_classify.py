...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool3d((1, 1, 1)),  # GAP Layer
            nn.Flatten(),  # Flatten the output
            nn.Linear(feature_size, feature_size // 2),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(feature_size // 2, 4),  # For 4 classes
            nn.Softmax(dim=1)
        )
...
class SwinTransformerClassifier(nn.Module):
    def __init__(self, swin_config, num_classes):
        super(SwinTransformerClassifier, self).__init__()
        
        # Swin Transformer
        self.swin = SwinTransformer(**swin_config)
        
        # Feature Dimension of Swin's last layer
    ...
    def forward(self, x):
        ...
        features = self.swin(x)[-1]  # Use the last hidden state
        
        # Global Average Pooling
        gap = features.mean(dim=[-3, -2, -1])  # For 3D input
